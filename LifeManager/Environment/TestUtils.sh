...
# If they are not, the program will exit with an appropriate error message.
#
# 1 The expected return code value.
# 2 The name of the test.
function assert_return_code() {
  expected=$1
  function_handler=$2
  test_name=$3
  # Parameters to function handler = $4..$n

  validate_arg_count $# "${FUNCNAME[0]}" 4 255

  echo "Testing: $test_name"
  "$function_handler" "${@:4}" >>/dev/null
...
function assert_user_input_result() {
  ...
  echo "User input testing: $test_name"
  printf '%s' "$user_input" | "$function_handler" "${@:5}" >>/dev/null
